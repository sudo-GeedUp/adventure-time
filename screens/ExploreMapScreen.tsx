...
export default function ExploreMapScreen() {
  ...
  const [rawCommunityAdventures, setRawCommunityAdventures] = useState<any[]>(
    [],
  );
  ...
  const loadNearbyData = useCallback(async () => {
    ...
    // Load trails (sample + offline cached)
    let nearbyTrails = getTrailsNearLocation(coords, maxDistance);
    try {
  ...
  // Filter community adventures by distance and map to UI type
  useEffect(() => {
    const maxDistance =
      distanceFilter === "all" ? 50 : parseInt(distanceFilter);
    const coords = userLocation?.coords || {
    ...
    const adventuresData: Adventure[] = rawCommunityAdventures
      .filter((adv: any) => {
  ...
  }, [userLocation, distanceFilter, rawCommunityAdventures]);
