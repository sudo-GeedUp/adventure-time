...
async function api(method, path, body) {
  ...
  if (!res.ok)
    throw new Error(
      `${method} ${path} -> ${res.status}: ${JSON.stringify(json).slice(0, 1000)}`,
    );
  return json;
}
...
(async () => {
  ...
  for (const s of subs.data) {
    console.log(
      `${s.id} | ${s.attributes.productId} | state=${s.attributes.state}`,
    );
  }

  const groups = await api(
    "GET",
    `/apps/${APP_ID}/subscriptionGroups?limit=200`,
  );
  for (const g of groups.data) {
    ...
    const list = await api(
      "GET",
      `/subscriptionGroups/${g.id}/subscriptions?limit=200`,
    );
    for (const s of list.data) {
      console.log(
        `${s.id} | ${s.attributes.productId} | state=${s.attributes.state} | name=${s.attributes.name}`,
      );
    }
  }
