...
async function api(method, path, body) {
  ...
  if (!res.ok) throw new Error(`${method} ${path} -> ${res.status}: ${JSON.stringify(json).slice(0,2000)}`);
  return json;
}

(async () => {
  // List review submissions and their items
  const subs = await api('GET', `/reviewSubmissions?filter[app]=${APP_ID}&filter[platform]=IOS&limit=200&include=items`);
  for (const sub of subs.data || []) {
    console.log(`\nReviewSubmission ${sub.id} state=${sub.attributes?.state}`);
    const items = await api('GET', `/reviewSubmissions/${sub.id}/items?limit=200`);
    for (const item of items.data || []) {
      const type = item.type;
      const rel = item.relationships;
      console.log(`  Item ${item.id} type=${type} state=${item.attributes?.state}`);
      if (rel?.appStoreVersion?.data) console.log(`    appStoreVersion=${rel.appStoreVersion.data.id}`);
      if (rel?.inAppPurchase?.data) console.log(`    inAppPurchase=${rel.inAppPurchase.data.id}`);
      if (rel?.subscription?.data) console.log(`    subscription=${rel.subscription.data.id}`);
      if (rel?.appStoreVersionExperiment?.data) console.log(`    experiment=${rel.appStoreVersionExperiment.data.id}`);
    }
  }
