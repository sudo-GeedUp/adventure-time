...
(async () => {
  ...
  try {
    const items = await api('GET', `/reviewSubmissions/${SUBMISSION_ID}/items?limit=200`);
  ...
  } catch (err) { console.log('List items failed:', err.status); }
  ...
  } catch (err) { console.log('Cancel submission failed:', err.status); }
