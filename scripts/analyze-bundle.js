...
async function analyzeBundle(platform = 'all') {
  ...
  try {
    ...
    const platforms = platform === 'all' ? ['web', 'ios', 'android'] : [platform];
    const results = {};
      ...
      if (p === 'web') {
        ...
        if (fs.existsSync(mainBundle)) {
          ...
          if (bundleSize > THRESHOLDS.error) {
            colorLog('red', `⚠️  Bundle size exceeds recommended limit (${formatBytes(THRESHOLDS.error)})`);
          } else if (bundleSize > THRESHOLDS.warning) {
            colorLog('yellow', `⚠️  Bundle size is large (${formatBytes(THRESHOLDS.warning)})`);
          } else {
          ...
          jsFiles.slice(0, 5).forEach(file => {
            ...
            colorLog('bright', `  ${file.name}: ${file.sizeFormatted} (${percentage}%)`);
          });
        }
      } else {
        // For native platforms, we'd need to build and analyze the APK/IPA
        colorLog('yellow', `Native bundle analysis for ${p} requires full build. Run:`);
  ...
  for (const [platform, data] of Object.entries(results)) {
    if (data.totalSize > THRESHOLDS.error) {
      recommendations.push({
        ...
        details: `Bundle size (${formatBytes(data.totalSize)}) significantly exceeds recommended limit`
      });
    }
    ...
    if (data.files) {
      ...
      if (largeFiles.length > 0) {
        recommendations.push({
          ...
          details: largeFiles.map(f => `${f.name}: ${f.sizeFormatted}`).join(', ')
        });
      }
    }
  }
