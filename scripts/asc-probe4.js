...
(async () => {
  ...
  for (const b of builds.data || []) {
    ...
    console.log(`${b.id} | version=${attr.version} | processingState=${attr.processingState} | expired=${attr.expired} | uploaded=${attr.uploadedDate}`);
  }

  // Full app version with build include
  const version = await api('GET', `/appStoreVersions/ad6f2414-8b0d-4db1-a497-f3ac1a2d45c9?include=build`);
  console.log('\n=== Version 2.0.0(2) ===');
  console.log('attributes:', JSON.stringify(version.data.attributes, null, 2).slice(0,800));
  const buildIncluded = version.included?.find(x => x.type === 'builds');
  console.log('build included:', JSON.stringify(buildIncluded, null, 2).slice(0,800));
