...
async function main() {
  ...
  for (const v of versions.data || []) {
    console.log(`Version ${v.id}: ${v.attributes?.versionString} platform=${v.attributes?.platform} status=${v.attributes?.appVersionState} build=${v.relationships?.build?.data?.id}`);
  }

  console.log('\n=== Subscriptions ===');
  const subs = await api('GET', `/apps/${APP_ID}/subscriptions?limit=200`);
  for (const s of subs.data || []) {
    console.log(`Subscription ${s.id}: productId=${s.attributes?.productId} state=${s.attributes?.state} group=${s.relationships?.subscriptionGroup?.data?.id}`);
  }

  console.log('\n=== In-App Purchases ===');
  const iaps = await api('GET', `/apps/${APP_ID}/inAppPurchases?limit=200`);
  for (const i of iaps.data || []) {
    console.log(`IAP ${i.id}: productId=${i.attributes?.productId} referenceName=${i.attributes?.referenceName} state=${i.attributes?.state}`);
  }
}
