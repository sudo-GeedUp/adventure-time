...
export class ImageOptimizer {
   ...
   */
  static async getImageDimensions(uri: string): Promise<ImageDimensions | null> {
    try {
      if (Platform.OS === 'web') {
   ...
   */
  static async needsOptimization(uri: string, options: Partial<OptimizationOptions> = {}): Promise<boolean> {
  ...
  static async optimize(
    ...
    try {
      ...
      if (newWidth === dims.width && newHeight === dims.height && opts.quality === 1) {
        return uri;
      }
      ...
      if (Platform.OS !== 'web') {
        ...
        const manipulations = [{ resize: { width: newWidth, height: newHeight } }];
        ...
        console.log(`Image optimized: ${dims.width}x${dims.height} → ${newWidth}x${newHeight}`);
        return result.uri;
      } else {
...
export const generateThumbnail = ImageOptimizer.generateThumbnail.bind(ImageOptimizer);
export const getImageDimensions = ImageOptimizer.getImageDimensions.bind(ImageOptimizer);
