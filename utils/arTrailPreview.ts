...
class ARTrailPreviewManager {
  ...
  getVisibleMarkers(
    ...
    return markers.filter((marker) => {
      ...
      const normalizedBearing =
        relativeBearing > 180 ? 360 - relativeBearing : relativeBearing;
  ...
  calculateScreenPosition(
    ...
    const x =
      screenWidth / 2 +
      (relativeBearing / (fieldOfView / 2)) * (screenWidth / 2);
  ...
  getTrailDifficultyAssessment(
    ...
    if (
      !trail.vehicleTypes.includes(vehicleType) &&
      !trail.vehicleTypes.includes("Any 4WD")
    ) {
