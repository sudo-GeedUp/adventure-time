...
export class TrailDifficultyCalculator {
  ...
  static calculateDifficulty(
    ...
    const clearanceDiff =
      vehicleSpecs.groundClearance - trailRequirements.minGroundClearance;
    if (clearanceDiff < 0) {
    ...
    if (vehicleSpecs.wheelbase > trailRequirements.maxWheelbase) {
      const wheelbadeDiff =
        vehicleSpecs.wheelbase - trailRequirements.maxWheelbase;
    ...
    const approachDiff =
      vehicleSpecs.approachAngle - trailRequirements.minApproachAngle;
    if (approachDiff < 0) {
    ...
    const departureDiff =
      vehicleSpecs.departureAngle - trailRequirements.minDepartureAngle;
    if (departureDiff < 0) {
    ...
    const breakoverDiff =
      vehicleSpecs.breakoverAngle - trailRequirements.minBreakoverAngle;
    if (breakoverDiff < 0) {
    ...
    if (
      trailRequirements.maxVehicleWeight &&
      vehicleSpecs.weight > trailRequirements.maxVehicleWeight
    ) {
      score -= 10;
