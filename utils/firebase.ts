...
export class FirebaseLocationService {
  ...
  static async startLocationBroadcast(userId: string): Promise<void> {
    ...
    // Clear any existing broadcast before starting a new one
...
// Community Adventures - shared trips/trails
export class CommunityAdventuresService {
  static async publishAdventure(
    adventure: CompletedAdventure,
  ): Promise<string> {
    if (!database) throw new Error("Firebase not available");

    const adventuresRef = ref(database, "community-adventures");
    const newRef = push(adventuresRef);
    const adventureWithId = {
      ...adventure,
      id: adventure.id || newRef.key!,
      firebaseId: newRef.key!,
    };

    await set(newRef, adventureWithId);
    return newRef.key!;
  }

  static subscribeToCommunityAdventures(
    callback: (adventures: CompletedAdventure[]) => void,
  ): () => void {
    if (!database) {
      callback([]);
      return () => {};
    ...
    const adventuresRef = ref(database, "community-adventures");
    const unsubscribe = onValue(adventuresRef, (snapshot) => {
      const adventures: CompletedAdventure[] = [];
      snapshot.forEach((childSnapshot) => {
        adventures.push(childSnapshot.val() as CompletedAdventure);
      });
      adventures.sort((a, b) => b.endTime - a.endTime);
      callback(adventures);
    });

    return () => off(adventuresRef, "value", unsubscribe);
  }
}
...
export class ScanSubmissionsService {
  static async submitScan(
    ...
    const scansRef = ref(database, "scan-submissions");
    const newRef = push(scansRef);
  ...
  static subscribeToScanSubmissions(
    callback: (scans: SharedScanSubmission[]) => void,
  ): () => void {
    if (!database) {
      callback([]);
      return () => {};
    ...
    const unsubscribe = onValue(scansRef, (snapshot) => {
      const scans: SharedScanSubmission[] = [];
      snapshot.forEach((childSnapshot) => {
        scans.push(childSnapshot.val() as SharedScanSubmission);
      });
      callback(scans);
    });
